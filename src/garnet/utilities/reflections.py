...
IndexPeaks(PeaksWorkspace="peaks", RoundHKLs=False, Tolerance=0.1)
...
def AbsorptionCorrection(
    ...
    if shape == "sphere":
      ...
      '.format(
            params[0] / 200, params[1] / 100, alpha, beta, gamma
        )
    else:
        shape = ' \
        <cuboid id="cuboid"> \
    ...
    for peak in mtd[ws + "_corr"]:
        ...
        mu = mat.numberDensityEffective * (
            mat.totalScatterXSection() + mat.absorbXSection(wl)
        )
